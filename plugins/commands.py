...
def formate_file_name(file_name):
    ...
    for c in chars:
        file_name.replace(c, "")
    file_name = '@ClipMateBhai ' + ' '.join(filter(lambda x: not x.startswith('http') and not x.startswith('@') and not x.startswith('www.'), file_name.split()))
    return file_name
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton('🔍 sᴜᴘᴘᴏʀᴛ', url='https://t.me/Botmaster55'),
            InlineKeyboardButton('🤖 ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ', url='https://t.me/ClipMateBhai')
            ],[
    ...
    if data.split("-", 1)[0] == "verify":
        ...
        for msg in msgs:
            ...
            if info.media:
                ...
                if f_caption:
                    f_caption = f"@ClipMateBhai {f_caption.html}"
